...
def first_feauture_importance(input):
    df_sub = df.copy()
    X = array[:, 0:31]   # features
    y = array[:, 31]     # target - column "price"
    if input == 0:
        X = df_sub.values[:, 0:31]   # features
        y = df_sub.values[:, 31]     # target - column "price"
    else:
        df_sub = df[['Duration', 'Total_Stops', 'Additional_Info', 'day_sin',
                     'day_cos', 'month_sin', 'month_cos', 'Dep_hour_sin', 'Dep_hour_cos', 'Price']]
        X = df_sub.values[:, 0:9]   # features
        y = df_sub.values[:, 9]     # target - column "price"
    ...
    for f in range(X.shape[1]):
        print("%d. feature %d - column name = '%s' (%f)" % (f + 1,
              indices[f], df_sub.columns[indices[f]], importances[indices[f]]))
